...
This section follows the same step order as `DESIGN.md` Section 5. Each step includes a comparison table at the end showing the delta from the Azure-native design. Click any `SCN-###` to jump to the scenario trace in [Section 6](#6-scenario-traces).
...
**Microsoft SSE gap:** Entra Internet Access (SWG) does not include native RBI today. RBI requires a third-party vendor (Zscaler Cloud Browser Isolation, Palo Alto Prisma Browser Isolation, Menlo Security) integrated with the SASE PoP, or Microsoft Edge for Business session controls as a partial compensating control. This is captured in [Section 7](#7-requirements-traceability-matrix) and [Q-S08](#9-open-questions).
...
Roll these into a **3-5 year TCO** view: **Year 1** (implementation-heavy) vs **steady state** (licensing + operations). Tie governance rows above to **who bills** (cloud COE vs security vs network).

---

### Illustrative annual cost stab (Azure-native vs SASE)

**Not a quote.** Order-of-magnitude **USD/year** for planning conversations only. Replace every cell with your EA, SD-WAN partner, and Microsoft field pricing. Assumptions aligned to this design: **12 regions**, **24× Azure Firewall Premium** and **12× VWAN** hubs in Azure-native; **~2,500** combined **admin + ranger** SSE-eligible users; **12** corporate/legacy **SD-WAN sites** (SASE path); **ExpressRoute** treated as **4** active circuits (Azure-native admin primary) vs **internet underlay** on SASE; **RBI** on **~500** high-risk users if mandated.

| Cost bucket | Azure-native (illustrative) | SASE (illustrative) | Notes |
|---|---|---|---|
| **Azure Firewall Premium** (24 hubs) | ~$300k - $550k / yr | ~$0 | List-style: ~$1.0k - $1.9k / month / firewall × 24, varies by region and hours |
| **VWAN + VPN Gateway** (12 hubs, S2S + P2S scale) | ~$120k - $280k / yr | ~$0 | Hub scale units, gateways, data processing |
| **ExpressRoute** (4 circuits, blended) | ~$200k - $480k / yr | ~$40k - $120k / yr | SASE path assumes fewer dedicated circuits; retain some ER for gov/agency |
| **Azure landing zone** (monitoring, Policy, shared services) | ~$80k - $150k / yr | ~$90k - $170k / yr | Slightly higher Log Analytics if SASE log volume grows |
| **Microsoft SSE** (Entra Suite / GSA for ~2.5k users) | ~$0 - $60k / yr *(if already E5)* | ~$150k - $400k / yr | If not in E5, use **~$6 - $14 / user / month** band for SSE-class SKUs (illustrative) |
| **SD-WAN** (12 sites) | ~$0 (native path uses ER + VPN) | ~$35k - $110k / yr | **~$250 - $750 / site / month** CPE + orchestration (illustrative) |
| **Third-party FWaaS / IDPS** (if closing Microsoft SSE gap) | ~$0 | ~$0 - $200k / yr | Only if Zscaler/Prisma replaces or augments PoP IDPS |
| **RBI partner** (500 users) | ~$0 | ~$25k - $90k / yr | **~$4 - $15 / user / month** band (illustrative) |
| **Implementation (one-time, amortised rough Year-1 bump)** | ~$400k - $900k | ~$350k - $800k | Firewall/VWAN/ER commissioning vs SASE + connector + SD-WAN rollout; **not** annualised in table above |

**Rough total recurring (steady state, very wide bands):** **Azure-native ~$700k - $1.5M+ / yr** vs **SASE ~$350k - $900k / yr** before hybrid mitigations, with **SASE** shifting spend from **Azure network meters** to **per-user SSE + SD-WAN + optional RBI**. If GlobalParks **already** pays for **M365 E5** for all privileged users, move SSE from incremental to **$0 incremental** and the gap narrows toward **ER savings vs SD-WAN spend**.
...
| **Pro - Cost** | Entra Private Access and Entra Internet Access are included in M365 E5 or available as Entra Suite add-on. Avoids per-user SASE vendor licensing on top of Microsoft spend. |
